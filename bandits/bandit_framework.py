...
class Arm:
    def __init__(
        ...
        payout_function: Callable[[], float],
    ):
        self.arm_id = arm_id
        self.activation_probability = (
            activation_probability  # how likely is that the arm would yield any payout
        )
...
class Runner:
    ...
    def simulate(
        self, runs: int, epochs: int, recommendation_size: int
    ...
    def print_aggregated_results(self, aggregated: Dict[str, float]):
        print("\nAggregated results:\n")
        for bandit_id, total_payout in sorted(
            aggregated.items(), key=lambda x: x[1][-1], reverse=True
        ):
