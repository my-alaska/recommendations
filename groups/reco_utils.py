...
def _initialize_users(raw_ratings, k):
    ...
    users = pandas.DataFrame(
        2.0 * np.random.uniform(size=(users_no, k)) - 1.0,
        index=raw_ratings["userId"].unique(),
...
def _initialize_movies(raw_ratings, k):
    ...
    movies = pandas.DataFrame(
        2.0 * np.random.uniform(size=(movies_no, k)) - 1.0,
        index=raw_ratings["movieId"].unique(),
        columns=["x%s" % i for i in range(k)],
    )
...
def _calculate_user_preferences(
    users,
    movies,
    ratings,
    raw_ratings,
    ...
    while True:
        ...
        errors = np.where(
            ratings == 0.0,
            pandas.DataFrame(np.zeros((users_no, movies_no))),
...
def get_predicted_ratings(
    ...
    users_model, movies_model = _calculate_user_preferences(
        users,
        movies,
